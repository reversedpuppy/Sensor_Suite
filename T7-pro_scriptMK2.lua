...
local LW_upwelling_k1 =  9.007 
local LW_upwelling_k2 =  1.024

local LW_downwelling_k1 = 8.935
local LW_downwelling_k2 = 1.015

local SW_downwelling_factor = 24.16
local SW_upwelling_factor = 32.22

local rtc, err = MB.RA(61510, 0, 6)
if err ~= 0 then MB.W(6000, 1, 0) return end
prev_minute = rtc[5]
local filename = string.format("RMBL_ENV_DATA_MK1")
...
file:write("datetime,T_therm_dn,LWR_dn,T_therm_up,LWR_up,SWR_dn,SWR_up,snow_depth,wind_dir,wind_speed,Pressure,Humidity,H_temp\n")
file:close()
...
while true do
  ...
  if LJ.CheckInterval(1) then
    ...
    if rtc[5] ~= prev_minute then
      ...
      local ain64 = (ain(AIN_ADDR[9]) * 10) - 40 -- H temp
      ...
      local ain62 = (ain(AIN_ADDR[8]) * 10) - 25 -- Humidity
      ...
      writeval(file, ain62, true) -- pressure
